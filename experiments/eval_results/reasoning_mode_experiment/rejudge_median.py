...
def load_team():
    ...
    md = ROOT / 'sample_data' / 'sample_members'
...
fps = sorted(glob.glob(f'{EXP}/snapshots/wbs_snapshot_C3_3rounds_*reasoning_*.json'))
print(f"Found {len(fps)} v2 snapshots")
print(f"Judge model: {JUDGE_MODEL_GEMINI}")
rows = []
for i, fp in enumerate(fps, 1):
    fname = os.path.basename(fp)
    if 'reasoning_none' in fname: mode = 'R_none'
    elif 'reasoning_high' in fname: mode = 'R_high'
    elif 'reasoning_max' in fname: mode = 'R_max'
    else: mode = '?'

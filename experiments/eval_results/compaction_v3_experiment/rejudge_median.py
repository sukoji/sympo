"""Re-judge 9 v3 snapshots × 3 trials with Flash Lite (Pro Preview unstable)."""
...
os.environ['JUDGE_MODEL_GEMINI'] = 'gemini-3.1-flash-lite-preview'  # avoid Pro Preview hang
...
team = load_team()
fps = sorted(glob.glob(f'{EXP}/snapshots/wbs_snapshot_C3_3rounds_*compactv3_*.json'))
print(f"Found {len(fps)} v3 snapshots", flush=True)
rows = []
for i, fp in enumerate(fps, 1):
    fname = os.path.basename(fp)
    if 'compactv3_minimal' in fname: mode = 'C_minimal'
    elif 'compactv3_filter' in fname: mode = 'C_filter'
    elif 'compactv3_claude' in fname: mode = 'C_claude'
    else: mode = '?'

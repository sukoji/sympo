"""Compaction v4 — fixed C_claude trigger threshold (chars × 0.7, threshold 1500 tokens).

3 modes:
  C_minimal:  last 30 + PM 10
  C_filter:   last 8 + PM 4 (현 default)
  C_claude:   v4 fixed trigger (1500 tokens, chars*0.7) + REPLACE summary + last 6 + PM 4
"""
import sys, os
...
sys.path.insert(0, '/home/piai/ai_course/agent_test/eval_results/compaction_v4_experiment')
...
from compactor_claude_v4 import compactor_claude_v4
...
MODES = {'minimal': compactor_minimal, 'filter': compactor_filter, 'claude': compactor_claude_v4}

def _wrap_capture(mode_name, fn):
    samples_dir = '/home/piai/ai_course/agent_test/eval_results/compaction_v4_experiment/samples'
...
if __name__ == '__main__':
    ...
    print(f"🔧 Patched mode='{mode}' (v4: fixed trigger)", flush=True)
    os.environ['RUNNER_ID'] = f'compactv4_{mode}'

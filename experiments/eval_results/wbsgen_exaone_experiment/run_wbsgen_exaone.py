"""Run C3 with only the WBS generation agent routed to the EXAONE endpoint.
...
agents.wbs_gen_agent asks for model_config["wbs_gen"] == "exaone-wbs-api".

Usage:
  python eval_results/wbsgen_exaone_experiment/run_wbsgen_exaone.py [n_runs=3]
"""
import os
import sys
...
CONDITION_KEY = "H_wbsgen_exaone78b"
...
EXAONE_URL = os.getenv("WBS_EXAONE_API_URL", "http://127.0.0.1:8083")
EXAONE_MODEL = os.getenv("WBS_EXAONE_API_MODEL", "EXAONE-3.5-7.8B-Instruct-Q4_K_M.gguf")

_orig_get_llm = llm_config.get_llm


def _with_exaone_wbs_endpoint(temperature=0.7, max_tokens=1024):
    ...
    os.environ["QWEN_API_URL"] = EXAONE_URL
    os.environ["QWEN_API_MODEL"] = EXAONE_MODEL
...
def patched_get_llm(temperature=0.7, max_tokens=1024, model_id=None):
    if model_id == "exaone-wbs-api":
        print(f"DEBUG: WBS Gen routed to EXAONE endpoint {EXAONE_URL} ({EXAONE_MODEL})")
        return _with_exaone_wbs_endpoint(temperature=temperature, max_tokens=max_tokens)
...
def register_condition() -> str:
    ...
    cond.update(
        label="H: WBS Gen -> EXAONE 7.8B, others -> Gemma 26B",
        description="C3 with only initial WBS generation using EXAONE-3.5-7.8B; all later agents use Gemma-4-26B.",
        model_config={"wbs_gen": "exaone-wbs-api"},
    )
...
def main() -> None:
    ...
    os.environ["RUNNER_ID"] = "wbsgen_exaone78b"
    os.environ["LLM_BACKEND"] = BASE_BACKEND
    ...
    print("WBS Gen EXAONE experiment")
    print(f"Default agents: {GEMMA_URL} ({GEMMA_MODEL})")
    print(f"WBS Gen only:  {EXAONE_URL} ({EXAONE_MODEL})")

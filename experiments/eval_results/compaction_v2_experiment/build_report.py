"""Compaction v2 — final report + figures."""
import csv, glob, os
import sys
...
LABELS = {
    ...
    'C_filter':  'C_filter (sliding W=8 + PM 4) [현 default]',
    'C_claude':  'C_claude (4K threshold + LLM summary + cache, last 6)',
}
...
autoscore_rows = []
for path in glob.glob(f'{EXP}/summary_qwen-api_compactv2_*.csv'):
    mode = None
...
ax.set_title('Fig1. Compaction v2 — Overall (C3 3R, Gemma-4-26B, N=3, judge median ×3)')
...
ax.set_title('Fig3. Sub-agent Failure Rate by Compaction Mode (16K context overflow indicator)')
...
ax.set_title('Fig5. Compaction v2 — Overall Auto Score (C3 3R, Gemma-4-26B, N=3)')
ax.set_ylim(0, 1.0); ax.grid(axis='y', linestyle=':', alpha=0.4)
plt.tight_layout()
...
with open(f'{EXP}/COMPACTION_V2_REPORT.md', 'w') as f:
    f.write('# Compaction Strategy Experiment v2 — Final Report\n\n')
    f.write(f'생성일: {os.popen("date +%Y-%m-%d").read().strip()}  |  N=3, C3 3R, Gemma-4-26B, Pro Preview Judge (median ×3)\n\n')
    f.write('## 0. 사전 등록 가설\n\n')
    f.write('- **H1**: C_claude > C_filter > C_minimal (압축 정교화 → quality 향상)\n')
    f.write('- **H2**: latency C_claude > C_filter > C_minimal (요약 LLM 추가)\n')
    f.write('- **H3**: 모든 mode의 sub-agent failure rate < 10%/run (시스템 안전 영역)\n\n')
    f.write('## 1. 조건\n\n')
    f.write('| Mode | Trigger | 압축 방식 | 캐싱 | 최근 raw |\n|---|---|---|---|---|\n')
    f.write('| **C_minimal** | 매 호출 | last 30 raw + PM 10 (약압축, 안전 baseline) | ❌ | last 30 |\n')
    f.write('| **C_filter** (default) | 매 호출 | sliding W=8 + PM 4 | ❌ | last 8 |\n')
    f.write('| **C_claude** | est_tokens > 4K | Gemma26 self-summarization | ✅ | last 6 |\n\n')
    f.write('## 2. 통제\n\n')
    ...
    f.write(f'### H1 (C_claude > C_filter > C_minimal)\n')
    f.write(f'- C_minimal = {om:.3f}, C_filter = {of:.3f}, C_claude = {oc:.3f}\n')
    if oc > of > om:
        f.write(f'- ✅ 지지 (단조 순서 확인)\n\n')
    elif oc > om and of > om:
        f.write(f'- ⚠️ 부분 지지: 둘 다 minimal보다 높지만 filter vs claude 순서 ambiguous\n\n')
    else:
        f.write(f'- ❌ 기각\n\n')
    ...
    f.write(f'- C_minimal failures/run: {fm:.1f}, C_filter: {ff:.1f}, C_claude: {fc:.1f}\n')
    if max(fm, ff, fc) < 5:
    ...
    f.write('- Assignment 차원의 phantom-ID 문제로 A 점수가 모든 mode에서 0 가까움 — 압축 외 confound\n\n')
    f.write('## 8. 산출물\n\n')
    f.write('```\ncompaction_v2_experiment/\n')
    f.write('├── COMPACTION_V2_REPORT.md\n')
    f.write('├── summary_judged.csv          (9 runs × 3 judge trials each)\n')
    f.write('├── figures/fig1~6.png\n')
    f.write('├── snapshots/                  (9 wbs+debate JSON)\n')
    f.write('├── samples/                    (각 mode별 압축 입력 sample)\n')
    f.write('├── logs/                       (실행 로그)\n')
    f.write('├── run_compaction_v3.py        (재현 launcher)\n')
    f.write('├── rejudge_median.py           (3-trial median judge)\n')
    f.write('├── build_report.py             (이 figure+report 생성)\n')
    f.write('└── orchestrate_v2.sh           (전체 pipeline)\n')
    f.write('```\n')

print('✅ COMPACTION_V2_REPORT.md')

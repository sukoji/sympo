"""Re-judge ONLY claude_v3 snapshots × 3 trials each. Then merge into summary_judged.csv."""
...
team = load_team()
fps = sorted(glob.glob(f'{EXP}/snapshots/wbs_snapshot_C3_3rounds_*compactv2_claude_v3*.json'))
print(f"Found {len(fps)} claude_v3 snapshots")
new_rows = []
for i, fp in enumerate(fps, 1):
    ...
    print(f"\n[{i}/{len(fps)}] claude_v3 r{run_id}  n_tasks={len(wbs)}  failures={failures}")
    ...
    overall_med = sum(v*w for _,v,w in valid)/sum(w for _,_,w in valid) if valid else -1
    new_rows.append({
        'mode':'C_claude_v3','run_id':run_id,'snapshot':fname,'n_tasks':len(wbs),
    ...
    print(f"   → MEDIAN: S={sm} A={am} D={dm} → Overall={overall_med:.4f}")

# Append to existing summary_judged.csv
old_rows = list(csv.DictReader(open(f'{EXP}/summary_judged.csv')))
all_rows = old_rows + new_rows
out=f'{EXP}/summary_judged_v3.csv'
with open(out,'w',newline='') as f:
    w=csv.DictWriter(f, fieldnames=all_rows[0].keys())
    w.writeheader(); w.writerows(all_rows)
print(f"\n💾 Saved: {out} ({len(all_rows)} rows total)")

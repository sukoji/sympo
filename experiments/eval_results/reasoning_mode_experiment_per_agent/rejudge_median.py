...
EXP = '/home/piai/ai_course/agent_test/eval_results/reasoning_mode_experiment_per_agent'
...
fps = sorted(glob.glob(f'{EXP}/snapshots/wbs_snapshot_C3_3rounds_*reasoning_pa_*.json'))
print(f"Found {len(fps)} v2 snapshots")
rows = []
for i, fp in enumerate(fps, 1):
    fname = os.path.basename(fp)
    if 'reasoning_pa_baseline' in fname: mode = 'PA_baseline'
    elif 'reasoning_pa_wbs_high' in fname: mode = 'PA_wbs'
    elif 'reasoning_pa_super_high' in fname: mode = 'PA_super'
    elif 'reasoning_pa_subagents_high' in fname: mode = 'PA_subagents'
    elif 'reasoning_pa_all_high' in fname: mode = 'PA_all'
    else: mode = '?'
